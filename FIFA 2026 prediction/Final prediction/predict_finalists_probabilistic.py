"""
Probabilistic Monte-Carlo finalist prediction

This script is like `predict_finalists.py` but samples match winners probabilistically
using the model's predicted probability (instead of thresholding at 0.5). This adds
variance to simulations and produces more realistic distributions.

Usage:
  python predict_finalists_probabilistic.py --teams 16 --runs 2000 --seed 42

Outputs (CSV):
- predicted_finalists_frequency_prob.csv
- predicted_winners_frequency_prob.csv
- predicted_final_pairs_frequency_prob.csv
"""
...
def predict_match_win_prob(model, scaler, features_df, a, b):
...
def simulate_tournament_once_prob(model, scaler, features_df, teams, rng):
    order = list(teams)
    rng.shuffle(order)
    ...
    for group in groups:
        for i in range(len(group)):
            for j in range(i+1, len(group)):
                a, b = group[i], group[j]
                p = predict_match_win_prob(model, scaler, features_df, a, b)
                # probabilistic sampling: A wins with prob p, B wins with prob 1-p
                if rng.random() < p:
                    winner = a
                else:
                    winner = b
    ...
    # knockout with probabilistic sampling
...
def run_simulations(args):
    ...
    print(f"Using {len(teams)} teams for simulation. Running {args.runs} simulations (probabilistic)...")
    ...
    for run in range(args.runs):
        finals, champion = simulate_tournament_once_prob(model, scaler, features_df, teams, rng)
        if finals[0] is None:
            continue
        a, b = finals
    ...
    df_finalists.to_csv('predicted_finalists_frequency_prob.csv')
    df_winners.to_csv('predicted_winners_frequency_prob.csv')
    df_pairs.to_csv('predicted_final_pairs_frequency_prob.csv', index=False)

    print('Saved probabilistic results CSVs.')
    print('\nTop 10 finalists:')
...
if __name__ == '__main__':
    ...
    p.add_argument('--runs', type=int, default=2000, help='Number of Monte-Carlo tournament simulations')
